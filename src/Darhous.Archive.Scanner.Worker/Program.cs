...
public static class Program
{
    ...
    [System.Runtime.Versioning.RequiresPreviewFeatures("The default NAPS2 SDK adapter uses APIs marked preview by NAPS2.")]
    public static async Task<int> Main()
    {
        ...
        try
        {
            ...
            await using var transport = await NamedPipeTransport.ConnectClientAsync(pipe, options, connectionTimeout.Token);
            using var engine = new Naps2ScannerEngine();
            await new ScannerWorkerSession(engine, options).RunAsync(transport, token, shutdown.Token);
            return 0;
        }
        catch (OperationCanceledException) when (shutdown.IsCancellationRequested) { return 0; }
        catch (Exception)
        {
            // Never print exception messages that might contain launch credentials or frame contents.
            Console.Error.WriteLine("Scanner worker startup or transport failed.");
            return 1;
        }
    }
}
