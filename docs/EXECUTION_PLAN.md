...
- **المرحلة الحالية:** Phase 10 (Importers) مكتملة محليًا، 204/204 اختبار ناجح، Release build نظيف (0 warnings/errors). جاري تجهيز Commit+Push، ثم Phase 11 (Preview).
- **Repository:** https://github.com/Darhous/archive — Phase 0-9 مدفوعة على `main`، CI شغّال. Phase 10 لسه محليًا وقت كتابة السطر ده.
- **آخر مرحلة مكتملة (مدفوعة):** Phase 9 — Automatic Computer Discovery + Job System.
- **آخر مرحلة مكتملة (محليًا):** Phase 10 — Importers (204/204 اختبار، تفاصيل كاملة في قسم Phase 10 تحت). PDF/DOCX/XLSX/PPTX/EML/MSG بقى ليهم استخراج نص/Metadata حقيقي؛ Legacy DOC/XLS/PPT مؤجل لحد Phase 13 (Worker Infrastructure) لأسباب ترتيب حقيقية.
- **العمل القادم:** Commit + Push لـPhase 10، ثم Phase 11 — Preview (§59-60): PDF Preview، Office preview fallback، Metadata preview، Versions tab، Activity tab (lazy-loaded)، قاعدة تسجيل `preview_document` بدل `open_document`.
