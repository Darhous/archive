...
- **المرحلة الحالية:** Phase 18 (Backup & Restore) **مكتملة، Committed، Tagged (`phase-18`)، ومدفوعة فعليًا** — كل مراحل 0-18 دلوقتي مدموجة في `main`. Phase 19 (Updates) وPhase 20 (AI Foundation) قيد التجهيز لكودكس (مستقلتين عن بعض تمامًا — Updates عن آلية التحديث، AI Foundation عن تجريد مزوّد الذكاء الاصطناعي — هيتوزعوا على **جلستين Codex منفصلتين بالتوازي**، مش واحدة بعد التانية، بناءً على طلب Ahmed الصريح: كل مرحلة مستقلة تاخد جلسة Codex خاصة بيها عشان الـContext).
  - **قرار تنظيمي جوهري (2026-09-13)**: جيميني بقى في دور فحص شامل (Audit/QA) بس، مش تنفيذ مراحل — راجع [[feedback_orchestration_worktrees]]. أول تقرير فعلي وصل ونُقِد بالتفصيل في §8.1 تحت (3 نتائج حقيقية، 2 False Positive اتصلحوا بمراجعة الكود الفعلي). **درس إضافي مهم من مراجعة هذا التقرير**: حتى في دور الـAudit، أرقام الـBuild/Test بتاعت جيميني موثوقة 100%، لكن تصنيف الخطورة (Severity) لأنماط زي `!` محتاج تحقق فعلي من الـInvariant اللي بيضمن الأمان قبل قبوله — نفس مبدأ عدم الثقة العمياء، لكن مطبَّق على "التفسير" مش بس "الأرقام".
  - **Phase 19 (Updates)** و**Phase 20 (AI Foundation)** — بريفين جاهزين (`PHASE19_BRIEF.md`, `PHASE20_BRIEF.md`) من §78-80 (Updates: Check for updates/Manual install/Auto-check/Auto-install/Rollback، Update Safety: Verify signature→Backup DB if migration (يُعاد استخدام `Darhous.Backup.Local` من Phase 18 بدل اختراع آلية Backup تانية)→Stage→Rollback point، جدول `update_history` DB Spec §78) و§81-83 (AI Foundation: `IAiProvider` abstraction بس أولًا — العقد الحرفي معطى في الوثيقة الأم، AI settings عبر `app_settings` الموجود (مفيش جدول جديد)، Secrets عبر `ISecretProtector`/DPAPI الموجود من Phase 1، قاعدة صارمة: AI مايدخلش في Core Document Save transaction، Providers الرسمية (OpenAI/Gemini/Claude/OpenRouter) مؤجلة لمرحلة لاحقة — أول مرحلة تجريد بس). **لسه محتاجين Worktrees تتعمل قبل الإرسال لكودكس.**
  - **إجراء الدمج القياسي لأي مرحلة جاهزة** (اتبع بالظبط، زي Phase 13-18): اقرا `WORKER_REPORT.md` + راجع الكود الفعلي (مش تصدّق التقرير على العمى — [[feedback_orchestration_worktrees]])، شغّل build/test مستقل للتأكيد **لكل مشروع اختبار لوحده** (تشغيل الحل كله دفعة واحدة أثبت إنه ممكن يعلّق تحت حمل بيئي — راجع "درس Solution-wide test hang" تحت)، **بعد الدمج ابنِ الحل كله (مش بس المشاريع الجديدة) — تعديل توقيع Constructor في خدمة أساسية زي `JobRunner` ممكن يكسر مشروع اختبار قديم تمامًا في مرحلة تانية بدون أي علاقة، ده حصل فعليًا في دمج Phase 16 ولم يظهر إلا ببناء الحل بالكامل**، تأكد مفيش تصادم أرقام Migration مع مراحل تانية قيد التنفيذ بالتوازي أو مدموجة حديثًا (حصل 3 مرات متتالية Phase 15/16/18 — كل Worktree بيتفرّع من نقطة مختلفة وممكن ياخد نفس الرقم "التالي" اللي مرحلة تانية أخدته)، ادمج بـ`git checkout <branch> -- <paths>` في `main` (استبعد أي ملفات تجريبية/غير مقصودة من الـWorktree)، حدّث `Darhous.Archive.slnx` + قسم الـPhase هنا + سجل التقدم (§9) + هذا القسم (§1)، Commit+Tag(`phase-N`)+Push، اتأكد CI Success فعليًا بـ`gh run list` (مش مجرد الافتراض)، ثم احذف الـWorktree المدموج (`git worktree remove --force` — بيفشل غالبًا بـ"Permission Denied" أول مرة، لكن `git worktree prune -v` بعده بينظّف الميتاداتا حتى لو الـDirectory اتشال فعليًا أو لسه فاتح؛ إعادة محاولة حذف الـDirectory نفسه مش عاجلة).
- **Repository:** https://github.com/Darhous/archive — Phase 0-18 مدفوعين بالكامل على `main`. Tags: `phase-0` لغاية `phase-18` كلهم موجودين ومدفوعين. **CI: اتأكد من النتيجة بـ`gh run list` أول ما تُستأنف الجلسة (كان شغّال وقت آخر Push).**
- **آخر مرحلة مكتملة (مدفوعة):** Phase 18 — Backup & Restore.
