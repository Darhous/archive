...
- **المرحلة الحالية:** Phase 20 (AI Foundation) **مكتملة، Committed، Tagged (`phase-20`)، ومدفوعة فعليًا** — كل مراحل 0-20 دلوقتي مدموجة في `main`. Phase 22 (Performance Validation) شغّالة بالتوازي مع كودكس (مستقلة عن 19/20/21). Phase 21 (System Health) التالية، هتتجهّز دلوقتي بعد ما 19/20 اندمجوا فعليًا (كانت مؤجلة عمدًا لأنها بتعرض حالة AI/Updates/Backup).
  - **⚠️ أهم درس من هذه الجولة**: Phase 19 وPhase 20 اشتغلوا بالتوازي في جلستين Codex منفصلتين، وكل واحدة منهم اكتشفت بشكل مستقل نفس الفجوة القديمة (مفيش تنفيذ حقيقي لـ`IAppSettingsStore` من Phase 1/2) وبنت حل مختلف بنفس الاسم، وكمان الاتنين عدّلوا نفس ملفات واجهة Explorer لإضافة قوائمهم. **الدمج احتاج تدخل يدوي حقيقي** (مش `git checkout` بسيط) لدمج التعديلات على 4 ملفات مشتركة والتخلص من كلاس مكرر — تفاصيل كاملة في قسم Phase 20 تحت. **القاعدة الجديدة**: لما مرحلتين مستقلتين بالتوازي بيلمسوا ملفات الواجهة الرئيسية (`App.xaml.cs`/`ExplorerWindow`) أو بيسدّوا نفس الفجوة القديمة، الدمج الثاني محتاج مراجعة يدوية سطر-بسطر مش نسخ مباشر.
  - **قرار تنظيمي جوهري (2026-09-13)**: جيميني بقى في دور فحص شامل (Audit/QA) بس، مش تنفيذ مراحل — راجع [[feedback_orchestration_worktrees]]. أول تقرير فعلي وصل ونُقِد بالتفصيل في §8.1 (3 نتائج حقيقية، 2 False Positive اتصلحوا بمراجعة الكود الفعلي).
  - **Phase 21 (System Health)** — لسه محتاجة Worktree+Brief من §84-85 (صفحة Health تعرض Core/DB/Search/Scanner/OCR/Automation/Telegram/AI/Backup/Plugins، Actions: Retry/Restart worker/Rebuild index/View logs/Disable plugin/Open settings) — بتستخدم `IHealthContributor`/`IHealthRegistry` الموجودين من Phase 1، دلوقتي كل الأقسام اللي هتتعرض (AI من Phase 20، Backup من Phase 18، Updates من Phase 19) موجودة فعليًا فمفيش داعي لتأجيل تاني.
  - **إجراء الدمج القياسي لأي مرحلة جاهزة** (اتبع بالظبط، زي Phase 13-20): اقرا `WORKER_REPORT.md` + راجع الكود الفعلي (مش تصدّق التقرير على العمى — [[feedback_orchestration_worktrees]])، شغّل build/test مستقل للتأكيد **لكل مشروع اختبار لوحده**، **قبل نسخ أي ملف مشترك (`App.xaml.cs`/`ExplorerWindow`/`ServiceCollectionExtensions.cs`) اتأكد إن مرحلة تانية اندمجت حديثًا معدّلاه — لو كذلك، قارن الاتنين يدويًا بدل `git checkout` مباشر (زي دمج Phase 20)**، تأكد مفيش تصادم أرقام Migration، حدّث `Darhous.Archive.slnx` + قسم الـPhase + سجل التقدم (§9) + هذا القسم (§1)، Commit+Tag(`phase-N`)+Push، اتأكد CI Success فعليًا بـ`gh run list`، ثم `git worktree remove --force` + `git worktree prune -v`.
- **Repository:** https://github.com/Darhous/archive — Phase 0-20 مدفوعين بالكامل على `main`. Tags: `phase-0` لغاية `phase-20` كلهم موجودين ومدفوعين. **CI: اتأكد من النتيجة بـ`gh run list` أول ما تُستأنف الجلسة.**
- **آخر مرحلة مكتملة (مدفوعة):** Phase 20 — AI Foundation.
...
### Phase 20 — AI Foundation `[x]`
*مرجع: §81-83 — نُفِّذت بالكامل بواسطة Codex في Worktree مستقل (`phase-20-codex`)، رُوجِعت واندمجت بواسطة Claude*
- [x] مشروع جديد `Darhous.Archive.Modules.Ai`: `IAiProvider`/`AiModelInfo`/`AiRequest`/`AiResponse` مطابقة تمامًا لتوقيع §81، `IAiProviderRegistry` (Thread-safe، case-insensitive، مفيش Provider حقيقي مُسجَّل بعد — Registry جاهز فقط).
- [x] **AI settings عبر `app_settings` الموجود** (مفيش جدول/Migration جديد، بالظبط زي المطلوب) — Blob JSON واحد بالإصدار (Privacy acknowledgment + Active provider + Enabled flags لكل Provider).
- [x] **Secrets عبر `ISecretProtector`/DPAPI الموجود من Phase 1** (مفيش آلية سرية جديدة) — اتأكد بـRound-trip حقيقي لمفتاح تجريبي، والقيمة المخزنة في `app_settings` اتأكد إنها مش النص الصريح.
- [x] **Worker/Isolation boundary حقيقي**: قناة Bounded (Capacity 32) + Consumer واحد + Timeout قابل للتهيئة (30 ثانية افتراضي) + تنفيذ فعلي على Thread pool تاني غير الـCaller (مُثبَت باختبار). **قرار موثّق**: مفيش توجيه عبر Named Pipes/Worker خارجي زي Phase 13 لأنه هيحتاج اختراع عقد IPC/Provider-loading كامل بدون Provider حقيقي يستخدمه — Boundary الحالي قابل للاستبدال بـProcess حقيقي لاحقًا من غير ما يغيّر `IAiProvider`/`AiAssistService`.
- [x] **Privacy warning حقيقي على مستوى الـService مش بس الـUI**: تفعيل أي Provider من غير `PrivacyAcknowledged=true` مرفوض من الـService نفسه (مُثبَت باختبار)، مطابق لقاعدة §8 "Cloud AI = Disabled افتراضيًا دائمًا".
- [x] **`AiFailureRule` (§83) مُثبَتة بالمستوى المتاح فقط**: `AiAssistService` بيرجّع `NoEnrichment` لأي فشل (Exception/Timeout/Settings failure) بدل ما ينشر الخطأ — **مفيش ربط فعلي بـ`DocumentService`/Core Document Save transaction في المرحلة دي عمدًا** (النطاق المطلوب: تجريد فقط، الربط الفعلي لمرحلة لاحقة).
- [x] **واجهة إعدادات AI عربية** رقيقة، Admin-only، متصلة بقايمة Explorer.
- [x] **17 اختبار جديد** (14 AI + 1 Persistence + 2 Desktop).
- **⚠️ تعارض حقيقي مع Phase 19 (اكتُشف واتحل وقت الدمج)**: الاتنين اكتشفوا بشكل مستقل نفس الفجوة (مفيش Implementation حقيقي لـ`IAppSettingsStore`) وبنوا كلاسين مختلفين بنفس الاسم `SqliteAppSettingsStore` في مكانين مختلفين، وكمان عدّلوا نفس ملفات الواجهة (`App.xaml.cs`, `ExplorerWindow.xaml`/`.xaml.cs`) لإضافة قوائمهم الخاصة. **الدمج اليدوي**: نسخة Phase 19 اتقبلت كمرجعية (`IClock`-based)، نسخة Phase 20 اتشالت بالكامل (كودها الفعلي بيعتمد على الـInterface بس، مش الكلاس المحدد، فمفيش أي كسر)، وملفات الواجهة الأربعة اتدمجت يدويًا (مش `git checkout` مباشر) عشان تحتفظ بإضافات المرحلتين مع بعض. اتأكد بالبناء الكامل + كل الاختبارات إن الدمج اليدوي سليم 100%.
- **فجوات موثّقة صراحة (نفس مستوى الصدق المعتاد)**: Confidence **5.5/10 Production-readiness** (مفيش Provider حقيقي، مفيش اختبار شبكة، مفيش Process containment حقيقي، الفشل في بناء الحل الكامل بسبب امتلاء القرص أثناء نسخ ملفات اختبار Plugins — بيئي مش كودي) مقابل **8.5/10 Happy-path**.
- **تحقق مستقل مطابق 100%**: 14/14 `Ai.Tests`، 24/24 `Desktop.Tests` (22 قديم + 2 جديد)، 43/43 `Persistence.Tests`، Release build نظيف بعد الدمج اليدوي الكامل.
...
## 8.1 نتائج فحص جيميني الشامل (Audit، 2026-09-13)

بعد قرار تحويل دور جيميني من تنفيذ مراحل مستقلة لفحص شامل (Audit/QA — راجع [[feedback_orchestration_worktrees]])، نفّذ فحصًا كاملًا للحل عند `main` (Commit `0bc15b1`، Phase 0-15+17). النتيجة (`GEMINI_AUDIT_REPORT.md`) بُنيت على Build نظيف (0 تحذيرات) و313/313 اختبار ناجح (مطابق تمامًا لأرقامي المستقلة). **راجعت النتائج بنفسي (مش قبول أعمى) ولقيت الآتي**:

**حقيقية وتم تسجيلها كدين تقني:**
1. **(Critical، حقيقي 100%)** إنشاء مباشر (`new`) لخدمات أساسية (`JobRunner`, `SqliteUnitOfWork`, `InMemoryEventBus`) في عدة ملفات اختبار عبر مراحل مختلفة (`JobRunnerTests.cs`, `DiscoveryTestBase.cs`, `InMemoryEventBusTests.cs`, `OutboxEventBusTests.cs`, `SearchTestBase.cs`) بدل DI/Factory مركزي — هذا بالظبط السبب الجذري وراء الـRegression اللي اكتشفته في دمج Phase 16 (`DiscoveryTestBase.cs`). **قرار**: مش هيتصلح دلوقتي (Refactor واسع لكل ملفات الاختبار مخاطرة أكبر من الفايدة في نص التنفيذ)، لكن اتسجّل كقاعدة دائمة إلزامية: أي تعديل على Constructor خدمة أساسية لازم `grep` شامل قبل الدمج (اتسجّلت في §1 بالفعل).
2. **(Note، حقيقي جزئيًا)** `Microsoft.Extensions.Configuration` في `Directory.Packages.props` **مُثبَّت فعلًا بدون أي استخدام** في أي `.csproj` (تحقّق بـ`grep` مباشر) — تنظيف بسيط مؤجل، مش عاجل. **ملاحظة**: ادّعاء التقرير الإضافي إن `coverlet.collector` كمان غير مُستخدَم كان **خطأ** — تحقّقت ولقيته مُستخدَم فعليًا في 3 مشاريع اختبار.
3. **(Note، حقيقي ومعروف بالفعل)** تغطية اختبارات رقيقة في `Modules.Scanner.Tests` (2) و`Modules.Ocr.Tests` (5) — موثّق بالفعل في تقارير Phase 14/15 الأصلية، مش اكتشاف جديد لكن تأكيد مستقل مفيد.

**False Positives (اتحقّقت بالكود الفعلي ورُفضت)**:
4. **(كان مُصنَّف Major)** استخدام `_boundConnection!`/`_connectionFactory!` في `AppUserRepository`/`DocumentRepository` — ده **نمط Dual-mode متعمَّد وموثَّق من Phase 2** (نفس نمط `RoleRepository` — تعليق `///` صريح في الكود نفسه)، الـInvariant بيضمن إن واحد بس من الاتنين non-null حسب الـConstructor المُستخدَم. مش خطر حقيقي.
5. **(كان مُصنَّف Major)** استخدام `result.Error!.Message` في الـViewModels (`ExplorerViewModel`, `LoginViewModel`, `OnboardingViewModel`) — تحقّقت من `Result.cs`: `Error` مضمون non-null دائمًا لما `IsSuccess == false` (Invariant في الـConstructor نفسه)، وكل استخدام فعليًا محمي بفحص `IsSuccess`/`IsFailure` قبله مباشرة. مش خطر حقيقي.

**الدرس المُستفاد**: جيميني في دور الـAudit أفضل بكتير من دور التنفيذ (Build/Test أرقامه مطابقة 100%)، لكن لسه بيحتاج نفس درجة التشكك في "التفسير" (مش الأرقام الخام) — بيرفع Severity لأنماط `!` من غير ما يتحقق من الـInvariant اللي بيضمن أمانها. **القاعدة**: اقرا الكود المُشار إليه فعليًا قبل قبول أي Severity من تقرير Audit، مش بس ثِق في وصف "ليه ده خطر".
...
2026-09-13 — Phase 18 (Backup & Restore) مكتملة، مباشرة بعد Phase 16 (Phase 17 كانت اندمجت قبلها): `Darhous.Backup.Local` (metadata/full/configuration)، SQLite Online Backup API لكل الـ3 قواعد، Manifest+SHA-256+ZIP ذري، Restore بـSafety-backup تلقائي + Maintenance-lease جديدة على `SqliteWriteQueue` توقف كل طوابير الكتابة الداخلية وقت الاستعادة، Rollback تلقائي للفشل المبكر، `RestoreRecoveryRequiredException` صريح للفشل المتأخر (بدل استمرار بحالة مشكوك فيها). تصادم Migration تالت من نفس الفئة (`202609120006`→`202609120007`، Phase 16 كانت حجزت الرقم أثناء تنفيذ Phase 18 بالتوازي). 8 اختبارات جديدة + تحقق مستقل مطابق 100% (8/8، 42/42 Persistence، 22/22 Desktop). بالتوازي، أول تقرير Audit شامل من جيميني في دورها الجديد وصل ({313/313} اختبار مطابق، Build نظيف) — من 7 نتائج، 3 حقيقية (أهمها: بناء خدمات أساسية مباشرة في اختبارات متعددة، نفس سبب Regression Phase 16) و2 False Positive (أنماط `!` كانت محمية بـInvariants موثّقة، اتحقّق منها بالكود الفعلي مش بالوصف). تفاصيل كاملة في §8.1. جاري commit+tag(`phase-18`)+push، بعدها تجهيز Phase 19 (Updates) وPhase 20 (AI Foundation) لكودكس — مستقلتين عن بعض، هيتوزعوا على جلستين منفصلتين بالتوازي.
2026-09-13 — Phase 19 (Updates) اندمجت: حزم `.darhousupdate` موقّعة (CMS/PKCS#7 مُعاد استخدامها من Phase 12)، Update Safety Flow بالترتيب المطلوب (Backup حقيقي من Phase 18 قبل أي تحديث فيه Migration)، Rollback حقيقي بمستوى الملفات، `update_history` (`M202609120008`)، وأول Implementation حقيقي لـ`IAppSettingsStore` (كان بس عقد وجدول من غير كود فعلي من Phase 1/2!). 9 اختبارات جديدة بشهادات RSA حقيقية. تحقق مستقل مطابق 100%. Phase 20 (AI Foundation) اندمجت فورًا بعدها: `IAiProvider` مطابق للعقد الحرفي، Registry، Worker boundary حقيقي (Bounded channel + Timeout + Thread تاني)، Privacy gate على مستوى الـService، AI settings عبر `app_settings` الموجود (مفيش جدول جديد). **دمج معقّد حقيقي**: الاتنين (19 و20) اكتشفوا بالتوازي نفس فجوة `IAppSettingsStore` وبنوا كلاسين مختلفين، وعدّلوا نفس 4 ملفات واجهة (`App.xaml.cs`/`ExplorerWindow.xaml`/`.xaml.cs`/`.csproj`) — اتحل بمراجعة ودمج يدوي سطر-بسطر (مش `git checkout` تلقائي)، مع الإبقاء على نسخة Phase 19 من `SqliteAppSettingsStore` كمرجعية وحذف نسخة Phase 20 المكررة (كودها بيعتمد على الـInterface بس فمفيش كسر). 17 اختبار جديد. تحقق مستقل كامل بعد الدمج اليدوي: Build نظيف + كل الاختبارات مطابقة (14 Ai، 24 Desktop، 43 Persistence، 9 Updates، 11 Plugins). جاري commit+tag لكل مرحلة+push، بعدها Phase 21 (System Health — دلوقتي كل الأقسام اللي هتتعرض فيها موجودة) وPhase 22 (Performance Validation، شغّالة بالتوازي فعلًا مع كودكس منذ البداية لأنها مستقلة).
```
