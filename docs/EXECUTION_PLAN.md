...
- **المرحلة الحالية:** Phase 17 (Reports & Export) **مكتملة، Committed، Tagged (`phase-17`)، ومدفوعة فعليًا** (`51c2b02`)، CI شغّال حاليًا للتحقق. Phase 15 (`270741a`) وPhase 17 كلاهما اندمجا في نفس الجلسة، بترتيب Tag غير متسلسل عمدًا (`phase-15` ثم `phase-17`، لأن Phase 16 اتنفّذت بالتوازي ولسه مش مدموجة — هتاخد `phase-16` وقت دمجها لاحقًا رغم إنها اتعملت قبل Phase 17 زمنيًا).
  - **Phase 16 (Notifications) — لسه مش مدموجة، جاهزة تقنيًا بس محتاجة خطوتين ميكانيكيتين وقت الدمج**: جيميني في `C:\Users\ahmed\Desktop\Darhous-Archive-phase16-gemini` (Branch: `phase-16-gemini`، آخر Commit `68dc35c`، بعد Amend لتصحيح مشكلتين حقيقيتين اكتُشفتا بمراجعتي: (1) `Darhous.Notifications.Windows` معندوش يتبني (CS1061) — جيميني غيّر TFM لـ`net10.0-windows10.0.17763.0` وحل مشكلة توافق الحزمة؛ (2) Confidence 8/10 من غير Test حقيقي. **تحققت بنفسي بالبناء الفعلي لكل الأربع مشاريع بعد التصحيح**: الأربعة بيتبنوا نظيف (0 تحذيرات)، و44/44 اختبار بيعدي فعليًا لما يتشغّل كل مشروع لوحده (`Notifications.Tests`=1، `Windows.Tests`=1، `NotificationsTelegram.Tests`=1، `Persistence.Tests`=41 — الأخير بيتأكد إن تعديل `JobRunner.cs` (إضافة `IEventBus`) مسبّبش Regression). **ملاحظة صادقة لازم تتسجل وقت الدمج**: تغطية الاختبارات لأول 3 مشاريع رقيقة جدًا (اختبار واحد لكل مشروع) رغم حجم الكود (Repository/Service/Provider/Plugin) — مقبولة للدمج بس مش "100% verified" زي ما وصفها جيميني. **خطوتين ميكانيكيتين متبقيتين وقت الدمج**: (أ) `M202609120005_Notifications` لازم ينترقّم لـ`202609120006` (نفس الرقم اللي Phase 15 استخدمته أصلًا بقى محجوز على `main`)، (ب) استبعاد ملفين تجريبيين مش لازمين اتضافوا بالغلط للـCommit (`test_reflect.cs`, `test_results.txt`) وقت الـ`git checkout <branch> -- <paths>`.
  - **إجراء الدمج القياسي لأي مرحلة جاهزة** (اتبع بالظبط، زي Phase 13/14/15/17): اقرا `WORKER_REPORT.md` + راجع الكود الفعلي (مش تصدّق التقرير على العمى — [[feedback_orchestration_worktrees]])، شغّل build/test مستقل للتأكيد **لكل مشروع اختبار لوحده** (تشغيل الحل كله دفعة واحدة أثبت إنه ممكن يعلّق تحت حمل بيئي — راجع "درس Solution-wide test hang" تحت)، تأكد مفيش تصادم أرقام Migration مع مراحل تانية قيد التنفيذ بالتوازي أو مدموجة حديثًا، ادمج بـ`git checkout <branch> -- <paths>` في `main` (استبعد أي ملفات تجريبية/غير مقصودة من الـWorktree)، حدّث `Darhous.Archive.slnx` + قسم الـPhase هنا + سجل التقدم (§9) + هذا القسم (§1)، Commit+Tag(`phase-N`)+Push، اتأكد CI Success فعليًا بـ`gh run list` (مش مجرد الافتراض)، ثم احذف الـWorktree المدموج (`git worktree remove --force` — لاحظ إنه فشل 3 مرات بـ"Permission Denied" لـPhase 14/15/17 المدموجين، على الأغلب بسبب Terminal/Process لسه فاتح جواه؛ إعادة المحاولة لاحقًا مش عاجلة، بيضيع مساحة قرص بس).
- **Repository:** https://github.com/Darhous/archive — Phase 0-15 وPhase 17 مدفوعين بالكامل على `main` عند Commit `51c2b02` (Phase 16 لسه بره `main`). Tags: `phase-0` لغاية `phase-15` + `phase-17` كلهم موجودين ومدفوعين (`phase-16` هيتضاف وقت دمجها). **CI: شغّال حاليًا (in_progress) لآخر Push — اتأكد من النتيجة بـ`gh run list` أول ما تُستأنف الجلسة.**
- **آخر مرحلة مكتملة (مدفوعة):** Phase 17 — Reports & Export.
