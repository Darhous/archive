...
- **المرحلة الحالية:** Phase 14 (Scanner) **مكتملة، Committed، Tagged (`phase-14`)، ومدفوعة فعليًا** (`73e0330`). بعدها اتصلحت مشكلة CI حقيقية (Commit `e3d6fa5`، تفاصيل في "درس CI" تحت) — **CI بقى Success فعليًا اتأكد ده بـ`gh run list`**. دلوقتي **مرحلتين قيد المراجعة النهائية** (كل واحدة Worktree مستقل، مبني على `main` بعد Phase 14):
  - **Phase 15 (OCR)** — كودكس شغّال في `C:\Users\ahmed\Desktop\Darhous-Archive-phase15-codex` (Branch: `phase-15-codex`). **خلص فعليًا، 3 Commits** (`d28ecc8` feat، `357d53c` test hardening، `e0898a9` docs/report — آخر Commit في الفرع). راجعته بنفسي بالكامل: `WORKER_REPORT.md` من أدق التقارير لحد الآن (Confidence 4/10 Production-ready، 8/10 Happy-path — صادق ومفصّل)، اتأكدت مستقل (Build نضيف + 12/12 اختبار `Ocr.Worker.Tests` مطابق)، مفيش لمس لـ`slnx`/`EXECUTION_PLAN.md`، مفيش Push. **الـStretch goal (ربط `documents_fts.body`) اتعمل فعليًا مش مؤجل** (اتأكد بتقرير Codex نفسه: LEFT JOIN + اختبار End-to-end). **جاهز للدمج في `main`** — الخطوة التالية لما تُستأنف الجلسة: تنفيذ إجراء الدمج القياسي (تحت) لـPhase 15.
  - **Phase 16 (Notifications)** — جيميني شغّال في `C:\Users\ahmed\Desktop\Darhous-Archive-phase16-gemini` (Branch: `phase-16-gemini`، آخر Commit `4328811`). **مستقلة تمامًا عن Phase 15** (Outbox/EventBus من Phase 2). **مراجعتي كشفت مشكلتين حقيقيتين**: (1) مشروع `Darhous.Notifications.Windows` **معندوش يتبني خالص** (خطأ Compile حقيقي: `ToastContentBuilder` مفيهوش `Show()` مع حزمة `CommunityToolkit.WinUI.Notifications` 7.1.2) — مش "Timeout بيئي" زي ما تقرير جيميني ادّعى. (2) التقرير حط Confidence **8/10 من غير ما يشغّل Test واحد فعليًا** — بالظبط الغلطة اللي اتحذّر منها بالاسم في البريف بتاعه (كررها من Phase 14). بعتّله متابعة تصحيحية دقيقة (`PHASE16_FIX_REQUIRED.md` جوه الـWorktree + برومبت تنفيذ قصير) وباستنى رده. **لو الجلسة استؤنفت وانت مستني رد جيميني**: افحص `git log`/`git status` في الـWorktree ده الأول، واقرا `WORKER_REPORT.md` المحدَّث لو موجود، قبل أي حاجة تانية.
  - **إجراء الدمج القياسي لأي مرحلة جاهزة** (اتبع بالظبط، زي Phase 13/14): اقرا `WORKER_REPORT.md` + راجع الكود الفعلي (مش تصدّق التقرير على العمى — [[feedback_orchestration_worktrees]])، شغّل build/test مستقل للتأكيد، ادمج بـ`git checkout <branch> -- <paths>` في `main`، حدّث `Darhous.Archive.slnx` + قسم الـPhase هنا + سجل التقدم (§9) + هذا القسم (§1)، Commit+Tag(`phase-N`)+Push، اتأكد CI Success فعليًا بـ`gh run list` (مش مجرد الافتراض)، ثم احذف الـWorktree المدموج (`git worktree remove --force`) لتوفير مساحة القرص.
- **Repository:** https://github.com/Darhous/archive — Phase 0-14 مدفوعة بالكامل على `main` عند Commit `e3d6fa5` (بعد إصلاح CI). Tags من `phase-0` لغاية `phase-14` كلهم موجودين ومدفوعين. **CI: Success مؤكَّد** (آخر فحص فعلي بـ`gh run list`).
- **آخر مرحلة مكتملة (مدفوعة):** Phase 14 — Scanner.
