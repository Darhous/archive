...
- **المرحلة الحالية:** Phase 15 (OCR) **مكتملة، Committed، Tagged (`phase-15`)، ومدفوعة فعليًا** (`270741a`)، CI شغّال حاليًا للتحقق (`gh run list` — Push فوري). دلوقتي **مرحلتين تانيين قيد المراجعة/التنفيذ**:
  - **Phase 16 (Notifications)** — جيميني في `C:\Users\ahmed\Desktop\Darhous-Archive-phase16-gemini` (Branch: `phase-16-gemini`، آخر Commit `68dc35c`، بعد Amend لتصحيح مشكلتين حقيقيتين اكتُشفتا بمراجعتي: (1) `Darhous.Notifications.Windows` معندوش يتبني (CS1061) — جيميني غيّر TFM لـ`net10.0-windows10.0.17763.0` وحل مشكلة توافق الحزمة؛ (2) Confidence 8/10 من غير Test حقيقي. **تحققت بنفسي بالبناء الفعلي لكل الأربع مشاريع بعد التصحيح**: الأربعة بيتبنوا نظيف (0 تحذيرات)، و44/44 اختبار بيعدي فعليًا لما يتشغّل كل مشروع لوحده (`Notifications.Tests`=1، `Windows.Tests`=1، `NotificationsTelegram.Tests`=1، `Persistence.Tests`=41 — الأخير بيتأكد إن تعديل `JobRunner.cs` (إضافة `IEventBus`) مسبّبش Regression). **ملاحظة صادقة لازم تتسجل وقت الدمج**: تغطية الاختبارات لأول 3 مشاريع رقيقة جدًا (اختبار واحد لكل مشروع) رغم حجم الكود (Repository/Service/Provider/Plugin) — مقبولة للدمج بس مش "100% verified" زي ما وصفها جيميني. **لسه فيه تصادم Migration محتمل**: جيميني استخدم `M202609120005_Notifications` بنفس الرقم اللي استخدمته لـPhase 15 (`202609120005`) — لازم ينترقّم لـ`202609120006` وقت الدمج. **كمان فيه ملفات تجريبية مش لازمة اتضافت بالغلط للـCommit** (`test_reflect.cs`, `test_results.txt`) — تتشال وقت الدمج. **جاهزة للدمج بعد معالجة النقطتين دول.**
  - **Phase 17 (Reports & Export)** — كودكس في `C:\Users\ahmed\Desktop\Darhous-Archive-phase17-codex` (Branch: `phase-17-codex`، Commit `bf968d7`، مستقلة عن Phase 16، مبنية على `73e0330`). بلّغ الانتهاء: Excel/CSV/PDF/Print/Audit export/تصدير عرض Explorer الحالي، Reports Tests 12/12، Desktop Tests 22/22، 0 تحذيرات/أخطاء (حسب تقريره — **لسه محتاج مراجعة مستقلة كاملة مني قبل الدمج**، `WORKER_REPORT.md` موجود في جذر الـWorktree).
  - **إجراء الدمج القياسي لأي مرحلة جاهزة** (اتبع بالظبط، زي Phase 13/14/15): اقرا `WORKER_REPORT.md` + راجع الكود الفعلي (مش تصدّق التقرير على العمى — [[feedback_orchestration_worktrees]])، شغّل build/test مستقل للتأكيد **لكل مشروع اختبار لوحده** (تشغيل الحل كله دفعة واحدة أثبت إنه ممكن يعلّق تحت حمل بيئي — راجع "درس Solution-wide test hang" تحت)، تأكد مفيش تصادم أرقام Migration مع مراحل تانية قيد التنفيذ بالتوازي، ادمج بـ`git checkout <branch> -- <paths>` في `main` (استبعد أي ملفات تجريبية/غير مقصودة من الـWorktree)، حدّث `Darhous.Archive.slnx` + قسم الـPhase هنا + سجل التقدم (§9) + هذا القسم (§1)، Commit+Tag(`phase-N`)+Push، اتأكد CI Success فعليًا بـ`gh run list` (مش مجرد الافتراض)، ثم احذف الـWorktree المدموج (`git worktree remove --force` — لاحظ إنه فشل مرتين بـ"Permission Denied" لـPhase 14/15 المدموجين، على الأغلب بسبب Terminal/Process لسه فاتح جواه؛ إعادة المحاولة لاحقًا مش عاجلة).
- **Repository:** https://github.com/Darhous/archive — Phase 0-15 مدفوعة بالكامل على `main` عند Commit `270741a`. Tags من `phase-0` لغاية `phase-15` كلهم موجودين ومدفوعين. **CI: شغّال حاليًا (in_progress) لآخر Push — اتأكد من النتيجة بـ`gh run list` أول ما تُستأنف الجلسة.**
- **آخر مرحلة مكتملة (مدفوعة):** Phase 15 — OCR.
- **درس جديد (2026-09-13): تشغيل اختبارات الحل كله دفعة واحدة (`dotnet test Darhous.Archive.slnx`) ممكن يعلّق فعليًا (مش بس يبطّئ) تحت حمل بيئي عالي** (تحقق Phase 16 كان شغّال بالتوازي على نفس الجهاز) — العملية فضلت شغّالة ~40 دقيقة باستهلاك CPU شبه صفري (0.1 ثانية CPU خلال 10 ثواني مراقبة)، اتقفلت يدويًا (`Stop-Process -Force`) واتأكد بديل ناجح: تشغيل كل مشروع اختبار لوحده (19 مشروع، 300/300 نجحوا). **القاعدة الجديدة**: لو تشغيل الحل كله دفعة واحدة أخد أكتر من ~10 دقائق من غير نتيجة، افترض إنه معلّق (تأكد بفحص استهلاك CPU الفعلي مش بس إنه "شغّال")، اقفله، وشغّل كل مشروع اختبار لوحده بدل ما تستنى.
